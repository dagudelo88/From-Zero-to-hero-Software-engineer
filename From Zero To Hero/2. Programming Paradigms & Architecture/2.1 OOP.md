...
It lets you write code that works with **many different types** through a **single, consistent interface** (method name or contract), while each type provides its own specialized behavior.

There are two main flavors:

| Type                  | When Resolved     | Python                          | TypeScript                              | Real Power |
|-----------------------|-------------------|---------------------------------|-----------------------------------------|----------|
| **Compile-time** (Static) | At compile time  | Not native (can fake with `*args` / `**kwargs`) | Native function overloading            | Type safety + better DX |
| **Run-time** (Dynamic)   | At runtime       | Method overriding + **Duck Typing** | Method overriding + **Structural Typing** | Maximum flexibility & extensibility |

The **real magic** in most modern applications is **run-time polymorphism** — it’s what makes systems like payment processors, plugin architectures, game engines, and API handlers so powerful.

#### Real-world analogies (choose your favorite)

1. **Remote Control** (classic)  
   Same “Power” button → TV turns on screen, AC starts cooling, Ceiling Fan starts spinning. The button doesn’t care *what* it controls.

2. **Payment at Checkout**  
   You say “pay $49.99”. The system doesn’t care if it’s a credit card, PayPal, Apple Pay, or bank transfer — each method knows how to do its job.

3. **Shapes on a Canvas**  
   `draw()` on a `Circle`, `Square`, or `Triangle` — same method name, completely different rendering logic.

4. **Game Enemies**  
   Every enemy has `attack()` — a Goblin swings a club, a Wizard casts a spell, a Dragon breathes fire.

#### Code Examples (Production-Ready Style)

##### Python – Duck Typing + Inheritance (The Pythonic Way)
...
# 1. Classic inheritance + abstract base (recommended for larger systems)
...
class CreditCardProcessor(PaymentProcessor):
    def process(self, amount: float) -> str:
        return f"✓ Charged ${amount:.2f} to credit card ending in 4242"

class PayPalProcessor(PaymentProcessor):
    def process(self, amount: float) -> str:
        return f"✓ Paid ${amount:.2f} via PayPal (txn: PAY-XYZ123)"

class ApplePayProcessor(PaymentProcessor):
    def process(self, amount: float) -> str:
        return f"✓ Apple Pay authorized ${amount:.2f}"

# 2. Pure Duck Typing (very Pythonic – no inheritance required)
class CryptoProcessor:
    def process(self, amount: float) -> str:
        return f"✓ Sent {amount} USDC on Ethereum"

# 3. Polymorphic function / service
def checkout(processor: PaymentProcessor | object, amount: float) -> None:
    """Works with ANY object that has a .process() method"""
    print(processor.process(amount))

# Usage – completely interchangeable
checkout(CreditCardProcessor(), 129.99)
checkout(PayPalProcessor(), 49.50)
checkout(ApplePayProcessor(), 19.99)
checkout(CryptoProcessor(), 250.00)   # ← Duck typing in action!
```

##### TypeScript – Interfaces + Overloading (Best of Both Worlds)

```typescript
// 1. Interface (structural typing = TypeScript's duck typing)
...
class CreditCardProcessor implements PaymentProcessor {
    process(amount: number): string {
        return `✓ Charged $${amount.toFixed(2)} to credit card ending in 4242`;
    }
}

class PayPalProcessor implements PaymentProcessor {
    process(amount: number): string {
        return `✓ Paid $${amount.toFixed(2)} via PayPal (txn: PAY-XYZ123)`;
    }
}

// 2. Compile-time overloading (TypeScript only)
class Calculator {
...
// 3. Polymorphic service with dependency injection
class CheckoutService {
    constructor(private processor: PaymentProcessor) {}

    checkout(amount: number): void {
        console.log(this.processor.process(amount));
    }
}
...
| Pitfall                              | Bad Example                              | Good Fix |
|--------------------------------------|------------------------------------------|----------|
| **Violating Liskov Substitution**    | `Penguin.fly()` returns `"I can't fly"` when parent promises flying | Use composition or separate `Swimmable` / `Flyable` interfaces |
| **Tight coupling to concrete classes** | `if isinstance(processor, CreditCardProcessor)` | Always program to the interface/protocol |
| **Overriding without calling super** | Child completely ignores parent logic    | Call `super().method()` when it makes sense |
| **TypeScript: Wrong interface shape**| Class has `processPayment()` instead of `process()` | Use `implements PaymentProcessor` strictly |

**Golden Rule**:  
**Child classes should be substitutable for their parents without breaking expectations.**

#### Mermaid Diagram (Clean Architecture View)
...
classDiagram
    ...
    ApplePayProcessor ..|> PaymentProcessor
    CheckoutService --> PaymentProcessor : "depends on abstraction"
```

#### Industry Pro-Tip: Why This Matters in Real Systems

- **Python**: Duck typing lets you add new payment methods, notification channels, or data exporters **without touching existing code**. This is why Django, FastAPI, and pandas feel so extensible.
- **TypeScript**: Interfaces + structural typing give you **compile-time safety** while still allowing the same flexibility. Perfect for large enterprise codebases and micro-frontend architectures.
- **Design Pattern Connection**: Polymorphism is the foundation of the **Strategy**, **Factory**, and **Decorator** patterns.

**Bottom line**:  
Master polymorphism and you stop writing `if-elif` chains for every new type. Instead, you write **one** piece of code that works with **any** future implementation that follows the contract.
