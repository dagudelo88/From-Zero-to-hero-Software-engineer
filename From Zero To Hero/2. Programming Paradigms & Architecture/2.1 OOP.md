...
**Level 3 (Deep): Method Resolution Order (MRO) & the Diamond Problem**

When inheritance gets complex—especially with multiple parents—Python must decide **which method to call** when the same name exists in several places. It uses the **C3 linearization algorithm** (the same algorithm used by languages like Perl and Dylan) to create a strict, predictable **Method Resolution Order (MRO)**.

The classic challenge is the **Diamond Problem**:
...
class Child(Parent1, Parent2):   # Creates a diamond shape
    pass
```
...
- Always inspect `YourClass.__mro__` (or use `help(YourClass)`) when using multiple inheritance.
- Prefer **cooperative multiple inheritance** with `super()` instead of hard-coded parent calls.
- Python’s MRO guarantees a **linear** order, eliminating ambiguity.
...
Here’s a small but realistic zoo system that demonstrates single, multilevel, and multiple inheritance in one cohesive codebase:
...
class Penguin(Bird, Mammal):     # Hybrid: Bird + Mammal
    def speak(self):
...
# Usage
buddy = Dog("Buddy", 3)
pingu = Penguin("Pingu", 5)

print(buddy.eat())      # Inherited from Animal
print(buddy.nurse())    # Inherited from Mammal
print(pingu.fly())      # Inherited from Bird
print(pingu.swim())     # New method
print(pingu.speak())    # Overridden
```

This structure is clean, reusable, and easy to extend—exactly what inheritance is designed for.

**Best Practice Tip**: Use `super()` when overriding methods so child classes remain cooperative with their parents:
```python
class Dog(Mammal):
    def eat(self):
        return super().eat() + " (and loves treats!)"
```
