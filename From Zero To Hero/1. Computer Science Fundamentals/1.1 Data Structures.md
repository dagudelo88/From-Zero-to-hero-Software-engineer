...
```mermaid
graph LR
    subgraph Enqueue ["Enqueue (Enter Back)"]
    A[Value: 50] --> B("Queue: 40, 30, 20, 10")
    end
    subgraph Dequeue ["Dequeue (Exit Front)"]
...
```mermaid
graph LR
    subgraph Level 3 ["Level 3 (Fastest)"]
    A3[1] ---------> D3[30] ---------> F3[null]
    end
    subgraph Level 2
    A2[1] ----> C2[15] ----> D2[30] ----> F2[null]
    end
    subgraph Level 1 ["Level 1 (Slowest / Complete)"]
